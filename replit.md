...
- **Settlement Engine:** Production-grade settlement orchestration in `src/services/settlement/`. Services: `settlementService` (obligation lifecycle, idempotency), `settlementLedgerService` (immutable movement ledger), `settlementBatchService` (daily batch process/settle/fail), `settlementReconciliationService` (partner-reported vs internal comparison), `settlementAlertsService` (overdue/mismatch/exposure alerts). Firestore collections: `se_obligations`, `settlement_batches`, `settlement_movements`, `settlement_alerts`, `settlement_reconciliation_reports`. Admin screens: `AdminSettlementOverviewScreen`, `AdminSettlementEngineObligationsScreen`, `AdminSettlementBatchesScreen`, `AdminSettlementAlertsScreen`, `AdminSettlementReconciliationScreen`.
